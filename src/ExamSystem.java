...
/**
 * The ExamSystem class is the "Engine" of the application.
 */
public class ExamSystem extends JFrame {
    ...
    // MODIFIED: Constructor now accepts the department to filter questions
    public ExamSystem(String studentName, String department) {
        ...
        // --- 4. DATABASE & TIMER LOGIC (UPDATED) ---
        
        // Use the new filtered method you created in DatabaseManager
    ...
    private void checkAnswer() {
        String[] keys = {"A", "B", "C", "D"};
        for (int i = 0; i < 4; i++) {
            if (options[i].isSelected() && keys[i].equals(questionList.get(currentIdx).correct)) {
                score++;
            }
        }
    }
